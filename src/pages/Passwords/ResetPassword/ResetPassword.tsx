...
import { staffPasswordUrl, loginStaff, forgotPassword } from 'routes';
      ...
      apiServer
        ...
        .catch((err) => {
          ...
          if (err?.response?.data?.code === 2010) {
            history.push(forgotPassword, 'resetPasswordExpired');
          }
          console.error(err.response.data.error);
          setErrMsg(t('Error.server_down_error'));
        });
    } else {
      setErrMsg(t('Error.no_reset_token_provided'));
    }
  };
        ...
        {!showRedirectMsg && (
          ...
          >
            ...
            {submit ? (
              ...
              <input
                type='submit'
                value={t('Reset_password.button_text')}
          ...
          <div className='min-w-full my-16'>
            <div className='w-9/12 mx-auto mt-0 mb-10 text-xs font-light text-black 2xl:w-7/12 md:mb-24 md:mt-2 md:text-base'>
              <p className='w-full whitespace-pre-line'>
                {t('Password_reset_confirm.message')}
              </p>
            </div>
            <input
              type='button'
